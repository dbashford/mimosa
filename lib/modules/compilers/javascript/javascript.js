...
module.exports = JSCompiler = (function() {
      ...
      return _this.compile(file, function(err, output, sourceMap) {
        ...
        if (err) {
          ...
          if (sourceMap) {
            ...
            fileUtils.writeFile(mapFileName, sourceMap, function(err) {
              if (err) {
                return logger.error("Error writing map file [[ " + mapFileName + " ]], " + err);
              }
            });
            sourceName = _this._genSourceName(config, file);
            fileUtils.writeFile(sourceName, file.inputFileText, function(err) {
              if (err) {
                return logger.error("Error writing source file [[ " + sourceName + " ]], " + err);
              }
            });
  ...
  JSCompiler.prototype._genMapFileName = function(config, file) {
    ...
    return file.inputFileName.replace(extName, ".map").replace(config.watch.sourceDir, config.watch.compiledDir);
  };
