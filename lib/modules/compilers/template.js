"use strict";
var TemplateCompiler, fileUtils, fs, logger, path, _, __generateTemplateName, __removeClientLibrary, __templatePreamble, __testForSameTemplateName, _init,
...
_init = function(config, options, next) {
  options.destinationFile = function(compilerName, folders) {
...
module.exports = TemplateCompiler = (function() {
  ...
  TemplateCompiler.prototype._removeFiles = function(config, options, next) {
    ...
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      outputFileConfig = _ref[_i];
      _results.push(__removeClientLibrary(options.destinationFile(this.compiler.name, outputFileConfig.folders), done));
    }
    return _results;
  };
