...
MimosaCompilerModule = (function() {
  ...
  function MimosaCompilerModule() {
    this._testDifferentTemplateLibraries = __bind(this._testDifferentTemplateLibraries, this);    this.all = wrench.readdirSyncRecursive(__dirname).filter(function(f) {
  ...
  MimosaCompilerModule.prototype.validate = function(config, validators) {
    ...
    if (validators.ifExistsIsObject(errors, "template config", config.template)) {
      ...
      validTCompilers = ["handlebars", "dust", "hogan", "jade", "underscore", "lodash", "ejs", "html", "emblem"];
