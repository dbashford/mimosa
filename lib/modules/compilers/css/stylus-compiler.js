...
module.exports = StylusCompiler = (function(_super) {
  ...
  StylusCompiler.prototype.importRegex = /@import[\s\t]*[\(]?[\s\t]*['"]?([a-zA-Z0-9*\/\.\-\_]*)[\s\t]*[\n;\s'")]?/g;
  ...
  StylusCompiler.prototype.compile = function(file, config, options, done) {
    ...
    stylusSetup = this.compilerLib(text).include(path.dirname(fileName)).include(config.watch.sourceDir).set('compress', false).set('filename', fileName).set('include css', true);
  ...
  StylusCompiler.prototype._determineBaseFiles = function(allFiles) {
