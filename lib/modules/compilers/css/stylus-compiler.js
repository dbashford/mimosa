"use strict";
var AbstractCssCompiler, StylusCompiler, fs, logger, path, stylus, _,
...
module.exports = StylusCompiler = (function(_super) {
  ...
  StylusCompiler.prototype.compile = function(file, config, options, done) {
    var cb, fileName, stylusSetup, text,
      _this = this;
    ...
    stylusSetup = stylus(text).include(path.dirname(fileName)).include(config.watch.sourceDir).set('compress', false).set('firebug', (config.isOptimize != null) && !config.isOptimize).set('filename', fileName);
    if (config.stylus.resolvedUse.length > 0) {
      stylusSetup.use.apply(stylusSetup, config.stylus.resolvedUse);
    }
    if (config.stylus["import"].length > 0) {
      stylusSetup["import"].apply(stylusSetup, config.stylus["import"]);
    }
