...
module.exports = StylusCompiler = (function(_super) {
  ...
  StylusCompiler.prototype.compile = function(file, config, options, done) {
    var cb, fileName, text, _ref, _ref1,
      _this = this;
    ...
    return (_ref = (_ref1 = stylus(text).include(path.dirname(fileName)).include(config.watch.sourceDir).set('compress', false).set('firebug', (config.isOptimize != null) && !config.isOptimize).set('filename', fileName)).use.apply(_ref1, config.stylus.resolvedUse))["import"].apply(_ref, config.stylus.use).render(cb);
  };
