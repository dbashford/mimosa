...
_compile = function(file, config, options, done) {
  ...
  stylusSetup = _compilerLib(text).include(path.dirname(fileName)).include(config.watch.sourceDir).set('compress', false).set('filename', fileName).set('include css', true);
