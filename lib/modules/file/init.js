"use strict";
var fileUtils, fs, logger, path, _determineDestinationFile, _initMultiAsset, _initSingleAsset,
  __slice = [].slice;
...
_initSingleAsset = function(config, options, next) {
  ...
  logger.debug("Destination for file [[ " + (inputFile != null ? inputFile : "template file") + " ]] is [[ ??? ]]");
...
exports.registration = function(config, register) {
  ...
  return register(['add', 'update', 'remove', 'cleanFile', 'buildFile'], 'init', _initSingleAsset, __slice.call(e.javascript).concat(__slice.call(cExts)));
};
