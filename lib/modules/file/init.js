...
MimosaFileInitModule = (function() {
  ...
  MimosaFileInitModule.prototype.registration = function(config, register) {
    ...
    register(['add', 'update', 'remove', 'cleanFile', 'buildFile'], 'init', this._initSingleAsset, __slice.call(e.javascript).concat(__slice.call(cExts)));
    return register(['add', 'update', 'remove', 'cleanFile', 'buildExtension'], 'init', this._initMultiAsset, __slice.call(e.template).concat(__slice.call(e.css)));
  };
  ...
  MimosaFileInitModule.prototype.__determineDestinationFile = function(config, options) {
    var compiledJSDir, destFunct, destinationFile, ext, exts,
      _this = this;

    exts = config.extensions;
    ext = options.extension;
    options.destinationFile = exts.template.indexOf(ext) > -1 ? (options.isTemplate = true, compiledJSDir = config.watch.compiledJavascriptDir, function(compilerName, folders) {
