"use strict";
var Module, PRECOMPILE_FUN_REGION_END_RE, PRECOMPILE_FUN_REGION_LINES_MAX, PRECOMPILE_FUN_REGION_SEARCH_LINES_MAX, PRECOMPILE_FUN_REGION_START_RE, baseDefaults, fs, logger, moduleManager, path, processConfig, validators, wrench, _, _applyAndValidateDefaults, _extend, _extractPrecompileFunctionSource, _findConfigPath, _moduleDefaults, _requireConfig, _setModulesIntoConfig, _validateSettings, _validateWatchConfig;
...
_extend = function(obj, props) {
  Object.keys(props).forEach(function(k) {
    var val;
    val = props[k];
    if ((val != null) && (typeof val === 'object') && (!Array.isArray(val)) && (!(val instanceof RegExp)) && (typeof obj[k] === typeof val)) {
...
_validateWatchConfig = function(config) {
  var currVersion, errors, i, isHigher, js, jsDir, minVersionPieces, piece, ss, vPiece, versionPieces, _i;
  errors = [];
  ...
  if (validators.ifExistsIsBoolean(errors, "watch.usePolling", config.watch.usePolling)) {
    if (process.platform !== 'win32' && config.watch.usePolling === false) {
      logger.warn("You have turned polling off (usePolling:false) but you are on not on Windows. If you\nexperience EMFILE issues, this is why. usePolling:false does not function properly on\nother operating systems.");
    }
  }
...
_validateSettings = function(config, modules) {
  var errors, mod, moduleErrors, _i, _len;
...
_setModulesIntoConfig = function(config) {
  ...
  _ref = moduleManager.installedMetadata;
  _results = [];
  for (_i = 0, _len = _ref.length; _i < _len; _i++) {
    mod = _ref[_i];
    _results.push(config.installedModules[mod.name] = mod.mod);
  }
  return _results;
};
