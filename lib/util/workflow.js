...
module.exports = WorkflowManager = (function() {
  ...
  WorkflowManager.prototype._buildAssetOptions = function(fileName) {
    ...
    ext = ext.length > 1 ? ext.substring(1) : (baseName = path.basename(fileName), console.log("CHECK IF IS DOTFILE???", baseName), baseName.indexOf(".") === 0 && baseName.length > 1 ? (console.log("IS DOT FILE!"), baseName.substring(1)) : '');
    return {
      inputFile: fileName,
      extension: ext
    };
