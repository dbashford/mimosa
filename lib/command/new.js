var NewCommand, buildConfig, compilers, deps, exec, fs, logger, path, servers, setupData, views, wrench, _,
...
NewCommand = (function() {
  ...
  NewCommand.prototype._usingNoServer = function() {
    ...
    this.config = this.config.replace(/\ \ #?\ ?server:[A-Za-z0-9 \"#\n\r:,.'-]*\n/g, "");
    this.config = this.config.replace(/\ \ #?\ ?liveReload:[\]\[A-Za-z0-9 \"#\n\r:,.'-]*/g, "");
    this.config = this.config.replace("  # modules:", "  modules:");
    this.config = this.config.replace(/(\ \ modules:\ [\[\]\'A-Za-z\,\ -]*)('server', )([\[\]\'A-Za-z\,\ -]*)/g, "$1$3");
    this.config = this.config.replace(/(\ \ modules:\ [\[\]\'A-Za-z\,\ -]*)('live-reload', )([\[\]\'A-Za-z\,\ -]*)/g, "$1$3");
    return this._done();
  };
