var NewCommand, buildConfig, color, compilerCentral, deps, exec, fs, logger, path, wrench, _,
...
NewCommand = (function() {
  ...
  NewCommand.prototype["new"] = function(name, opts) {
    ...
    return this._projectPossibilities(function(compilers) {
      if (opts.defaults) {
        return _this._createWithDefaults(compilers, name);
      } else {
        return _this._prompting(compilers, name);
      }
    });
  };

  NewCommand.prototype._projectPossibilities = function(callback) {
