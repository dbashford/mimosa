...
register = (program, callback) ->
  program
    ...
    .action(callback)
    .on '--help', =>
      logger.green('  The update command is here to help you from having to deal with updating your node_modules')
      logger.green('  directory when Mimosa updates its functionality.  For instance, if Express updates to a new')
      logger.green('  version, this command will update your package.json and install new node packages so you can')
      logger.green('  take advantage of the new Express functionality without having to deal with it yourself.')
      logger.blue( '\n    $ mimosa update\n')
