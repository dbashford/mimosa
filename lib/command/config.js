...
copyConfig = function(opts) {
  var conf, currDefaultsPath, defaultsConf, mimosaConfigPath, outConfigText;
  if (opts.mdebug) {
    opts.debug = true;
    logger.setDebug();
  ...
  defaultsConf = "# THE FOLLOWING IS A COMMENTED VERSION OF THE mimosa-config.coffee WITH\n# ALL OF THE MOST RECENT DEFAULTS. THIS FILE IS MEANT FOR REFERENCE ONLY.\n\n" + conf;
...
register = function(program, callback) {
  var _this = this;
  return program.command('config').option("-D, --mdebug", "run in debug mode").description("copy the default Mimosa config into the current folder").action(callback).on('--help', function() {
