...
clean = function(opts) {
  var cleaner, configurer, fileUtils, fs, logger, wrench,
    _this = this;
  fs = require('fs');
  ...
  return configurer(opts, function(config, modules) {
    if (opts.force) {
      ...
      config.isClean = true;
      return cleaner(config, modules, function() {
...
register = function(program, callback) {
  return program.command('clean').option("-f, --force", "completely delete your compiledDir").option("-C, --cleanall", "deletes the .mimosa directory as part of the clean").option("-P, --profile <profileName>", "select a mimosa profile").option("-D, --mdebug", "run in debug mode").description("clean out all of the compiled assets from the compiled directory").action(callback).on('--help', function() {
    var logger;
    logger = require('logmimosa');
