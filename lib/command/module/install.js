...
install = function(name, opts) {
  var currentDir, dirName, err, exec, fs, logger, mimosaPath, pack, path, wrench, _doLocalInstall, _doNPMInstall, _doneNPMInstall, _installModule, _prepareForNPMInstall, _revertInstall, _testLocalInstall;
  path = require('path');
  fs = require('fs');
  ...
  _doneNPMInstall = function(name, oldVersion) {
    console.log("OLD VERSION FIRST!", oldVersion);
    return function(err) {
      var backupPath;
      console.log("OLD VERSION SECOND !", oldVersion);
      if (err) {
  ...
  _prepareForNPMInstall = function(name) {
    ...
    oldVersion = null;
    console.log("GETTING OLD VERSION");
    if (fs.existsSync(beginPath)) {
      ...
      console.log(mimosaPackage.dependencies);
...
register = function(program) {
  var _this = this;
  return program.command('mod:install [name]').option("-D, --mdebug", "run in debug mode").description("install a Mimosa module into your Mimosa").action(install).on('--help', function() {
    var logger;
    logger = require("logmimosa");
