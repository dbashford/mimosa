var list, logger, printResults, register;

logger = require("logmimosa");

printResults = function(mods, opts) {
  var asArray, color, data, dep, field, fields, gap, headline, installed, longestModName, m, mod, moduleMetadata, name, ownedMods, spaces, spacing, verbose, version, _i, _j, _k, _l, _len, _len1, _len2, _len3;
...
list = function(opts) {
  ...
  return exec("npm config get proxy", function(error, stdout, stderr) {
    ...
    return request.get("http://mimosa-data.herokuapp.com/modules", options, function(error, client, response) {
      var mods;
      if (error !== null) {
        console.log(error);
        return;
...
register = function(program) {
  return program.command("mod:list").option("-D, --mdebug", "run in debug mode").option("-v, --verbose", "list more details about each module").option("-i, --installed", "Show just those modules that are currently installed.").description("get list of all mimosa modules in NPM").action(list).on("--help", function() {
    logger = require("logmimosa");
