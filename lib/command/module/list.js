var color, http, list, logger, moduleMetadata, printResults, register, request, childProcess;
...
list = function(opts) {
  ...
  childProcess.exec('npm config get proxy', function(error, stdout, stderr){
    var options = {
    ...
    // request with or without proxy
    request(options, function(error, client, response){
      if(error !== null){
        console.log(error);
        return;
      }
      return printResults(JSON.parse(response), opts)
    });
  });
  /*
  return http.get("http://mimosa-data.herokuapp.com/modules", function(res) {
    var data;
    data = '';
...
register = function(program, callback) {
  var _this = this;
  return program.command('mod:list').option("-D, --debug", "run in debug mode").option("-v, --verbose", "list more details about each module").option("-i, --installed", "Show just those modules that are currently installed.").description("get list of all mimosa modules in NPM").action(callback).on('--help', function() {
