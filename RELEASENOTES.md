...
* All of the compilers will be converted to JavaScript from CoffeeScript.
...
* The Handlebars compiler will be broken into two modules, one for Handlebars and one for Ember-Handlebars.
* The Emblem compiler will NOT be broken into two modules. For now I'm going with the assumption that all Emblem users are also Ember users, so the Emblem module that is created will be one that assumes Ember.
* The mimosa-config generated by `mimosa new` will be a JavaScript file and be reduced down to only the needed config without comments.  A 2nd file with all the comments for all of the
